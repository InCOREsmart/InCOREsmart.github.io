...
function Tooltip({ content, children }: { content: React.ReactNode; children: React.ReactNode }) {
  return <div className="group relative inline-flex w-full">{children}<div className="pointer-events-none absolute left-1/2 top-full z-50 mt-2 hidden w-72 -translate-x-1/2 rounded-xl bg-[#000052] px-4 py-3 text-left text-xs leading-relaxed text-white shadow-xl group-hover:block">{content}</div></div>;
}
...
export function AgentDashboard() {
  ...
  return <div className="p-4 md:p-6 space-y-6">
    ...
    <Tooltip content={<><div className="font-semibold mb-1">{t('agent.annualBonus')}</div><div>{t('tooltips.annualBonusAgent')}</div><div className="mt-1">{t('tooltips.annualBonusDescription')}</div><div className="mt-1">{t('agent.accumulated')}: <b>${annualBonus.accruedBonus.toLocaleString()}</b> {t('agent.maxBonus')} ${annualBonus.maxBonus.toLocaleString()}.</div></>}>
      <div className="bg-white p-6 rounded-xl border border-[#000052]/10 cursor-help"><div className="flex items-center justify-between gap-4 mb-3"><div><h2 className="text-lg font-bold text-[#000052] flex items-center gap-2"><Award className="w-5 h-5 text-[#B8860B]" />{t('agent.annualBonus')} · {annualBonus.year}</h2><p className="text-sm text-[#000052]/60 mt-1">{t('tooltips.annualBonusDescription')}</p></div><div className="text-right"><div className="text-xl font-bold text-[#000052]">${annualBonus.accruedBonus.toLocaleString()}</div><div className="text-xs text-[#000052]/60">{t('agent.maxBonus')} ${annualBonus.maxBonus.toLocaleString()}</div></div></div><div className="h-3 bg-[#000052]/10 rounded-full overflow-hidden"><div className="h-full bg-[#B8860B] rounded-full" style={{ width: `${Math.min(annualBonus.progressPercent, 100)}%` }} /></div><div className="flex justify-between mt-2 text-xs text-[#000052]/60"><span>{t('agent.planAchievement')}: {annualBonus.planAchievementPercent}%</span><span>{t('agent.accumulated')}: {annualBonus.progressPercent}%</span></div></div>
    </Tooltip>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><Tooltip content={<><div className="font-semibold mb-1">{t('agent.myActiveContracts')}</div><div>{t('tooltips.activeContractsAgent', { count: activeContractsCount, completed: completedContractsCount })}</div></>}><button onClick={() => navigate('/agent/contracts')} className="bg-white p-5 rounded-xl border border-[#000052]/10 hover:shadow-md transition text-left cursor-help"><div className="flex items-center gap-3 mb-2"><TrendingUp className="w-6 h-6 text-[#B8860B]" /><h3 className="font-bold text-[#000052]">{t('agent.myActiveContracts')}</h3></div><p className="text-sm text-[#000052]/70 mt-1">{activeContractsCount} {t('agent.activeContracts')}</p></button></Tooltip><Tooltip content={<><div className="font-semibold mb-1">{t('payouts.title')}</div><div>{t('tooltips.payoutsAgent')}</div></>}><button onClick={() => navigate('/agent/payouts')} className="bg-white p-5 rounded-xl border border-[#000052]/10 hover:shadow-md transition text-left cursor-help"><div className="flex items-center gap-3 mb-2"><DollarSign className="w-6 h-6 text-[#B8860B]" /><h3 className="font-bold text-[#000052]">{t('payouts.title')}</h3></div><p className="text-sm text-[#000052]/70">{t('payouts.subtitle')}</p></button></Tooltip></div>
  </div>;
}
