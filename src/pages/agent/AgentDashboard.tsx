...
function Tooltip({ content, children }: { content: React.ReactNode; children: React.ReactNode }) {
  return <div className="group relative inline-flex w-full min-w-0">{children}<div className="pointer-events-none absolute left-1/2 top-full z-50 mt-2 hidden w-[min(18rem,calc(100vw-2rem))] -translate-x-1/2 rounded-xl bg-[#000052] px-4 py-3 text-left text-xs leading-relaxed text-white shadow-xl group-hover:block">{content}</div></div>;
}
...
export function AgentDashboard() {
  ...
  return <div className="w-full min-w-0 max-w-full overflow-x-hidden p-3 sm:p-4 md:p-6 space-y-5 md:space-y-6">
    <div className="min-w-0"><h1 className="text-2xl md:text-3xl font-bold text-[#000052] break-words">{t('agent.dashboardTitle')}</h1><p className="text-sm text-[#000052]/70 mt-1 break-words">{agent.full_name}</p></div>
    <div className="bg-gradient-to-r from-[#000052] to-[#B8860B] text-white p-4 sm:p-5 rounded-xl min-w-0 overflow-hidden"><div className="flex items-start gap-3 min-w-0"><Shield className="w-6 h-6 flex-shrink-0 mt-0.5" /><div className="min-w-0"><h3 className="font-bold mb-1 break-words">{t('agent.fundsVerified')}</h3><p className="text-sm opacity-90 break-words">{t('agent.clawbackWarning')}</p></div></div></div>
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 min-w-0">{kpis.map((kpi, i) => { const Icon = kpi.icon; return <Tooltip key={i} content={kpi.detail}><div className={`${kpi.color} ${kpi.textColor} min-w-0 max-w-full p-4 sm:p-5 rounded-xl border border-[#000052]/10 cursor-help overflow-hidden`}><div className="flex items-start justify-between gap-2 mb-3"><h3 className="text-sm font-medium opacity-80 min-w-0 break-words">{kpi.label}</h3><Icon className="w-5 h-5 opacity-80 flex-shrink-0" /></div><p className="text-xl sm:text-2xl font-bold tabular-nums break-all">${kpi.value.toLocaleString()}</p></div></Tooltip>; })}</div>
    <Tooltip content={<><div className="font-semibold mb-1">{t('agent.annualBonus')}</div><div>{t('tooltips.annualBonusAgent')}</div><div className="mt-1">{t('tooltips.annualBonusDescription')}</div><div className="mt-1">{t('agent.accumulated')}: <b>${annualBonus.accruedBonus.toLocaleString()}</b> {t('agent.maxBonus')} ${annualBonus.maxBonus.toLocaleString()}.</div></>}>
      <div className="bg-white p-4 sm:p-5 md:p-6 rounded-xl border border-[#000052]/10 cursor-help min-w-0 max-w-full overflow-hidden"><div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-3"><div className="min-w-0"><h2 className="text-lg font-bold text-[#000052] flex items-start sm:items-center gap-2 break-words"><Award className="w-5 h-5 text-[#B8860B] flex-shrink-0 mt-0.5 sm:mt-0" />{t('agent.annualBonus')} · {annualBonus.year}</h2><p className="text-sm text-[#000052]/60 mt-1 break-words">{t('tooltips.annualBonusDescription')}</p></div><div className="text-left sm:text-right shrink-0 max-w-full"><div className="text-xl font-bold text-[#000052] tabular-nums break-all">${annualBonus.accruedBonus.toLocaleString()}</div><div className="text-xs text-[#000052]/60 break-words">{t('agent.maxBonus')} ${annualBonus.maxBonus.toLocaleString()}</div></div></div><div className="h-3 bg-[#000052]/10 rounded-full overflow-hidden"><div className="h-full bg-[#B8860B] rounded-full" style={{ width: `${Math.min(annualBonus.progressPercent, 100)}%` }} /></div><div className="flex flex-col sm:flex-row justify-between gap-1 mt-2 text-xs text-[#000052]/60"><span className="break-words">{t('agent.planAchievement')}: {annualBonus.planAchievementPercent}%</span><span className="break-words">{t('agent.accumulated')}: {annualBonus.progressPercent}%</span></div></div>
    </Tooltip>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 min-w-0"><Tooltip content={<><div className="font-semibold mb-1">{t('agent.myActiveContracts')}</div><div>{t('tooltips.activeContractsAgent', { count: activeContractsCount, completed: completedContractsCount })}</div></>}><button onClick={() => navigate('/agent/contracts')} className="bg-white p-4 sm:p-5 rounded-xl border border-[#000052]/10 hover:shadow-md transition text-left cursor-help min-w-0 max-w-full w-full overflow-hidden"><div className="flex items-start gap-3 mb-2"><TrendingUp className="w-6 h-6 text-[#B8860B] flex-shrink-0" /><h3 className="font-bold text-[#000052] break-words min-w-0">{t('agent.myActiveContracts')}</h3></div><p className="text-sm text-[#000052]/70 mt-1 break-words">{activeContractsCount} {t('agent.activeContracts')}</p></button></Tooltip><Tooltip content={<><div className="font-semibold mb-1">{t('payouts.title')}</div><div>{t('tooltips.payoutsAgent')}</div></>}><button onClick={() => navigate('/agent/payouts')} className="bg-white p-4 sm:p-5 rounded-xl border border-[#000052]/10 hover:shadow-md transition text-left cursor-help min-w-0 max-w-full w-full overflow-hidden"><div className="flex items-start gap-3 mb-2"><DollarSign className="w-6 h-6 text-[#B8860B] flex-shrink-0" /><h3 className="font-bold text-[#000052] break-words min-w-0">{t('payouts.title')}</h3></div><p className="text-sm text-[#000052]/70 break-words">{t('payouts.subtitle')}</p></button></Tooltip></div>
  </div>;
}
