    ...
    <DashboardLayout>
      ...
      <div className="mb-8">
        <h1 className="text-2xl md:text-3xl font-display font-bold text-text-primary">
          {t('dashboard.welcome')}, {agent?.full_name || t('auth.agent')}
        </h1>
      </div>

      {/* Stats Grid - Всегда виден */}
      <DashboardStats
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        {/* Левая часть: Активный контракт ИЛИ пустое состояние */}
          ...
          {latestContract ? (
            ...
            <div className="card text-center py-12">
              ...
              <p className="text-text-secondary">
                {t("agent.waitingTasks")}
              </p>
            </div>
          )}
        </div>

        {/* Правая часть: KPI Панель - ТЕПЕРЬ ОНА ВИДНА ВСЕГДА */}
        <div>
          <AgentKPIPanel />
        </div>
      ...
      {/* Payment Streams - Показываем только если есть контракты */}
      {contracts.length > 0 && latestContract && (
        <div className="card">
