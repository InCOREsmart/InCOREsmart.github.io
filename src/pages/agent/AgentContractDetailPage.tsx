...
export function AgentContractDetailPage() {
  ...
  const status = isCompleted ? t('agentContractDetail.completed') : isExpired ? t('agentContractDetail.expired') : t(`contract.statuses.${contract.status}`, contract.status);
  const healthLabel = health?.healthy ? t('smartContract.healthHealthy', 'Здоров') : t('smartContract.healthWarning', 'Требует внимания');
  const trustLabel = oracleTrust ? t(oracleTrust.labelKey, oracleTrust.level) : t('oracleTrust.untrusted', 'Не подтвержден');
  const streams = [
    { key: 'newSales', label: t('agentContractDetail.newSales'), value: model.newSales, detail: `Имущество $${model.newProperty} · КАСКО $${model.newCasco} · ДМС $${model.newDms}` },
    { key: 'renewal', label: t('agentContractDetail.renewal'), value: model.renewal },
    { key: 'crossSell', label: t('agentContractDetail.crossSell'), value: model.crossSell },
    { key: 'planBonus', label: t('agentContractDetail.planBonus'), value: model.planBonus },
    { key: 'retention', label: t('agentContractDetail.retention'), value: model.retention, detail: t('agentContractDetail.retentionCondition') },
    { key: 'annualBonus', label: t('agentContractDetail.annualBonus'), value: model.annualBonus, detail: `Накоплено ${annualBonus.progressPercent}% годового плана. Выплата в январе 2027.` },
  ];
