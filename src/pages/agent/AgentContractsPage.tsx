...
export function AgentContractsPage() {
  ...
  return <div className="p-4 md:p-8 space-y-6">
    <div>
      <h1 className="text-[26px] md:text-3xl font-bold text-[#000052] tracking-tight">{t('agent.myActiveContracts')}</h1>
    ...
    <div className="bg-[#000052] p-5 rounded-2xl shadow-[0_4px_16px_rgba(0,0,82,0.25)]">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="font-bold text-sm text-white">{t('agent.fundsVerified')}</p>
          <p className="text-xs text-white/70 mt-0.5">{t('agent.clawbackWarning')}</p>
        </div>
      </div>
    </div>
    ...
    {contracts.length === 0 ? (
      <div className="bg-white p-12 rounded-2xl shadow-sm text-center">
        ...
        <p className="text-lg font-semibold text-[#000052] mb-2">{t('ui.noContracts')}</p>
        <p className="text-sm text-gray-400">{t('ui.noContractsDescription')}</p>
      </div>
    ) : (
      <div className="space-y-5">
        {contracts.map(contract => {
          ...
          return (
            <div key={contract.id} className="bg-white rounded-2xl shadow-sm overflow-hidden transition-all duration-200 hover:shadow-[0_12px_40px_rgba(0,0,82,0.12)]">
              <div className="p-5 md:p-6">
                <div className="flex items-start justify-between mb-4 gap-3">
                  ...
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap ${statusInfo.color}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    ...
                    <div className="font-bold text-[#000052] mt-1">${(contract.revenue || 0).toLocaleString()}</div>
                  </div>
                  <div>
                    <div className="text-xs text-gray-400">{t('ui.escrow')}</div>
                    <div className="font-bold text-[#B8860B] mt-1">${(contract.escrow_amount || 0).toLocaleString()}</div>
                  </div>
                  <div>
                    <div className="text-xs text-gray-400">{t('ui.deadline')}</div>
                    <div className="font-bold text-[#000052] mt-1">{formatDate(contract.deadline)}</div>
                  </div>
                  <div>
                    <div className="text-xs text-gray-400">{t('ui.escrowStatus')}</div>
                    <div className="font-bold text-[#000052] mt-1 flex items-center gap-1.5">
                      {contract.escrow_status === 'FUNDED' ? (
                        <><Unlock className="w-3.5 h-3.5 text-emerald-600" />{t('ui.locked')}</>
                      ) : (
                        <><Lock className="w-3.5 h-3.5 text-gray-400" />{t('ui.notLocked')}</>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {streams.length > 0 && (
                <div className="px-5 md:px-6 py-4 bg-[#F4F5F7] border-t border-gray-100">
                  <div className="flex items-center justify-between">
                    <h4 className="text-sm font-bold text-[#000052]">{t('ui.paymentStreams')} ({streams.length})</h4>
                    <div className="text-xs text-gray-400">{t('ui.unlocked')}: ${unlockedAmount.toLocaleString()} / ${totalStreamAmount.toLocaleString()}</div>
                  </div>
                  <div className="h-[10px] bg-gray-200/70 rounded-full overflow-hidden my-4">
                    <div className="h-full bg-gradient-to-r from-[#B8860B] to-[#D4A017] rounded-full" style={{ width: `${totalStreamAmount > 0 ? (unlockedAmount / totalStreamAmount) * 100 : 0}%` }} />
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                    {streams.slice(0, 4).map(stream => (
                      <div key={stream.id} className="bg-white p-3 rounded-xl border border-gray-100">
                        <div className="text-gray-400 truncate">{stream.title}</div>
                        <div className="text-sm font-bold text-[#000052] mt-1">${stream.amount.toLocaleString()}</div>
                        <div className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide mt-1">{stream.status}</div>
                      </div>
                    ))}
                    {streams.length > 4 && (
                      <div className="bg-white p-3 rounded-xl border border-gray-100 flex items-center justify-center text-gray-400 font-semibold">+{streams.length - 4}</div>
                    )}
                  </div>
                  <div className="mt-4 p-4 bg-white rounded-xl border border-gray-100">
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        ...
                        <div className="text-sm font-bold text-emerald-600 mt-0.5">${paidAmount.toLocaleString()}</div>
                      </div>
                      <div className="w-px h-8 bg-gray-100"></div>
                      <div className="text-right">
                        <div className="text-xs text-gray-400">{t('ui.lockedAmount')}</div>
                        <div className="text-sm font-bold text-[#000052] mt-0.5">${lockedAmount.toLocaleString()}</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="p-4 flex gap-2 border-t border-gray-100">
                <button
                  ...
                  className="flex-1 py-2.5 px-4 bg-white border border-gray-200 text-[#000052] rounded-xl text-sm font-semibold hover:border-[#000052] hover:bg-[#000052]/5 transition-all duration-200"
                >
                  {t('ui.details')}
                </button>
                {(contract.status === 'ACTIVE' || contract.status === 'IN_PROGRESS') && (
                  <button
                    ...
                    className="flex items-center gap-2 py-2.5 px-4 bg-[#000052] text-white rounded-xl text-sm font-semibold shadow-[0_4px_16px_rgba(0,0,82,0.25)] hover:bg-[#14147a] hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200"
                  >
