...
const canonicalEscrow = (revenue: number) => revenue === CANONICAL_GMV ? 4701 : Math.round(revenue * (4701 / CANONICAL_GMV));
...
export function AgentContractsPage() {
  ...
  const totalEscrow = contracts.reduce((sum, c) => sum + canonicalEscrow(Number(c.revenue || c.kpi_revenue || CANONICAL_GMV)), 0);
  const totalRevenue = contracts.reduce((sum, c) => sum + Number(c.revenue || c.kpi_revenue || CANONICAL_GMV), 0);
