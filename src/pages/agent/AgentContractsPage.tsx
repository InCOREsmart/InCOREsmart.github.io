...
const getContractEscrow = (contract: any, streams: any[]) => {
  ...
  const streamEscrow = streams
    .filter((s) => s.stream_key !== 'annual')
    .reduce((sum, s) => sum + getStreamAmount(s), 0);
...
export function AgentContractsPage() {
  ...
  const [streamsByContract, setStreamsByContract] = useState<
    Record<string, any[]>
  >({});
  ...
  const handleAcceptContract = async (contractId: string) => {
    ...
    try {
      const { data: contract, error: contractError } = await supabase
        .from('contracts')
        .select(
          'id, company_id, title, status, escrow_status, agent_id'
        )
        .eq('id', contractId)
      ...
      if (!contract) {
        throw new Error(
          t('ui.contractNotFound', 'Контракт не найден')
        );
      }
      ...
      alert(
        t(
          'ui.contractAccepted',
          'Контракт принят и переведён в работу'
        )
      );
    } catch (err) {
      console.error(err);
      ...
      alert(
        t(
          'ui.contractAcceptError',
          'Не удалось принять контракт'
        )
      );
    } finally {
  ...
  const getStatusInfo = (status: string) => {
    ...
    > = {
      ...
      IN_PROGRESS: {
        icon: Send,
        color: 'bg-[#000052]/5 text-[#000052]',
        label: t('ui.inProgress'),
      },
      PENDING_APPROVAL: {
        icon: Clock,
        color: 'bg-[#B8860B]/10 text-[#B8860B]',
        label: t('ui.pendingApproval'),
      },
      COMPLETED: {
        icon: CheckCircle,
        color: 'bg-emerald-50 text-emerald-600',
        label: t('ui.completed'),
      },
      DISPUTED: {
        icon: AlertTriangle,
    ...
    >
          ...
          return (
            <div
              ...
              className="
                ...
                hover:shadow-[0_12px_40px_rgba(0,0,82,0.12)]
              "
            >
              <div
                className="
      ...
      <div
        className="
          ...
          sm:p-5
          rounded-2xl
          shadow-[0_4px_16px_rgba(0,0,82,0.25)]
          min-w-0
        "
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <p className="font-bold text-sm text-white break-words">
        ...
        <div className="space-y-4 sm:space-y-5">
          {contracts.map((contract) => {
            ...
            const streams = rawStreams
              .map((stream) => ({
                ...stream,
                amount: getStreamAmount(stream),
              }))
              .filter(
                (stream) =>
                  stream.stream_key !== 'annual'
              );

            const totalStreamAmount =
              getContractEscrow(
                contract,
                rawStreams
              );

            const unlockedAmount = streams
              .filter((stream) =>
                [
                  'UNLOCKED',
                  'PAYABLE',
            ...
            const lockedAmount = streams.length
              ? streams
                  .filter(
                    (stream) =>
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <div
                        ...
                        "
                      >
                        ${totalStreamAmount.toLocaleString()}
                      </div>
                    </div>
