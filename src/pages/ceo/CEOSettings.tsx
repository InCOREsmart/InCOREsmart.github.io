...
export function CEOSettings() {
      ...
      try {
        ...
        if (data) {
          ...
          setFormData(prev => ({
            ...prev,
            ...Object.fromEntries(
              Object.keys(prev).map(key => [key, data[key as keyof typeof data] ?? prev[key as keyof typeof prev]])
            )
          }));
        }
      } catch (err) {
  ...
  const field = (name: keyof typeof formData, label: string, required = false, type = 'text') => (
    <div>
      <label className="block text-sm font-medium text-[#000052] mb-2">{label}{required ? ' *' : ''}</label>
      <input name={name} type={type} value={formData[name]} onChange={handleChange} required={required} className="w-full px-4 py-3 bg-white border border-gray-200 rounded-[12px] text-[#000052] placeholder-gray-400 focus:ring-4 focus:ring-[#000052]/10 focus:border-[#000052] outline-none transition" />
    </div>
  );
    ...
    <form onSubmit={handleSave} className="space-y-6">
      <div className="bg-white p-6 rounded-2xl shadow-sm">
        <h2 className="text-lg font-bold text-[#000052] mb-5 flex items-center gap-3">
          <div className="w-10 h-10 rounded-[12px] bg-[#B8860B]/10 flex items-center justify-center flex-shrink-0"><Building2 className="w-5 h-5 text-[#B8860B]" /></div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm">
        <h2 className="text-lg font-bold text-[#000052] mb-5 flex items-center gap-3"><div className="w-10 h-10 rounded-[12px] bg-[#B8860B]/10 flex items-center justify-center flex-shrink-0"><User className="w-5 h-5 text-[#B8860B]" /></div>{t('company.personalData')}</h2>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm">
        <h2 className="text-lg font-bold text-[#000052] mb-5 flex items-center gap-3"><div className="w-10 h-10 rounded-[12px] bg-[#B8860B]/10 flex items-center justify-center flex-shrink-0"><Building2 className="w-5 h-5 text-[#B8860B]" /></div>{t('company.companyData')}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm">
        <h2 className="text-lg font-bold text-[#000052] mb-5 flex items-center gap-3"><div className="w-10 h-10 rounded-[12px] bg-[#B8860B]/10 flex items-center justify-center flex-shrink-0"><Landmark className="w-5 h-5 text-[#B8860B]" /></div>{t('company.bankDetails')}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
