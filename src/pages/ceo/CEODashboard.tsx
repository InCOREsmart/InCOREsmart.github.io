...
import { getActualContractRevenue, getContractAccountingSnapshot, money, PLATFORM_FEE_PERCENT } from '../../lib/contractFinance';
...
/**
 * CEO accounting rule:
 * revenue = actual amounts of client contracts/deals, never planned revenue.
 * payout = persisted payout streams, never a percentage guessed from GMV.
...
const getDashboardAccounting = (contract: any, streams: any[]) => {
  ...
  if (storedPayout > 0) {
    ...
    const paid = nonAnnual
      .filter((stream: any) => stream?.status === 'PAID')
      .reduce((sum: number, stream: any) => sum + money(stream?.amount), 0);
    const locked = nonAnnual
      .filter((stream: any) => stream?.status === 'LOCKED')
      .reduce((sum: number, stream: any) => sum + money(stream?.amount), 0);
    ...
    const companyProfit = money(revenue - storedPayout - commission);
  ...
  return {
    revenue: getActualContractRevenue(contract),
    escrow: snapshot.escrow,
    payout: snapshot.payout,
    paid: snapshot.paid,
    locked: snapshot.locked,
    liability: snapshot.locked,
    pending: 0,
    commission: snapshot.commission,
    companyProfit: snapshot.companyProfit,
  };
};
...
export function CEODashboard() {
      ...
      try {
        ...
        allContracts.forEach(contract => {
          ...
          if (accounting.revenue > 0) {
            roiSum += accounting.companyProfit / accounting.revenue * 100;
            roiCount += 1;
          }
          ...
          streams.filter((stream: any) => stream.stream_key !== 'annual').forEach((stream: any) => {
            ...
            if (d.getFullYear() === now.getFullYear() && d.getMonth() === now.getMonth()) {
              periodPaid += money(stream.amount);
            }
          });
        });
  ...
  const cards = [
    [t('ui.totalRevenue'), `$${Math.round(metrics.totalRevenue).toLocaleString()}`, DollarSign, 'bg-[#000052]', 'text-white', 'GMV: фактическая сумма договоров с клиентами по активным контрактам. Плановая выручка здесь не используется.'],
  ...
  return <div className="w-full min-w-0 overflow-x-hidden p-4 md:p-6 space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 min-w-0">
      ...
      <div className="bg-white min-w-0 p-5 rounded-xl border border-[#000052]/10"><p className="text-sm text-[#000052]/60 flex items-center break-words">{t('ui.released', 'Выплачено')}<InfoTooltip>Сумма потоков со статусом PAID по активным контрактам.</InfoTooltip></p><p className="text-2xl font-bold text-emerald-600 mt-1">${metrics.released.toLocaleString()}</p><p className="text-xs text-[#000052]/50 mt-1">Средства выплачены</p></div>
      <div className="bg-white min-w-0 p-5 rounded-xl border border-[#000052]/10"><p className="text-sm text-[#000052]/60 flex items-center break-words">{t('ui.liability', 'Обязательства')}<InfoTooltip>LOCKED + UNLOCKED + PAYABLE по активным контрактам. PAID сюда не входит.</InfoTooltip></p><p className="text-2xl font-bold text-[#000052] mt-1">${metrics.liability.toLocaleString()}</p><p className="text-xs text-[#000052]/50 mt-1">Текущие обязательства по выплатам</p></div>
    </div>

    <div className="bg-white min-w-0 p-4 md:p-6 rounded-xl border border-[#000052]/10">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-5"><div className="min-w-0"><div className="flex items-start gap-2"><Shield className="w-5 h-5 text-[#B8860B] shrink-0 mt-0.5" /><h2 className="text-lg font-bold text-[#000052] break-words">{t('ui.cashMovementAugust')}<InfoTooltip>Сравниваются выплаты текущего месяца и ожидаемые выплаты.</InfoTooltip></h2></div><p className="text-sm text-[#000052]/60 mt-1 break-words">{t('ui.cashMovementDescription')}</p></div><div className="text-left sm:text-right shrink-0"><div className="text-xs text-[#000052]/60 flex items-center sm:justify-end">{t('ui.inEscrow')}<InfoTooltip>Полный payout активных контрактов. Годовой бонус не входит в escrow.</InfoTooltip></div><div className="text-xl font-bold text-[#000052]">${metrics.totalEscrow.toLocaleString()}</div></div></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4 text-sm"><div><div className="text-[#000052]/60 flex items-center">{t('ui.escrow')}<InfoTooltip>Полный payout активных контрактов. Годовой бонус исключён.</InfoTooltip></div><div className="font-bold mt-1">${metrics.totalEscrow.toLocaleString()}</div></div><div><div className="text-[#000052]/60 flex items-center">{t('ui.paidInAugust')}<InfoTooltip>PAID-потоки, оплаченные в текущем месяце.</InfoTooltip></div><div className="font-bold text-emerald-600 mt-1">${metrics.periodPaid.toLocaleString()}</div></div><div><div className="text-[#000052]/60 flex items-center">{t('ui.pendingPayout')}<InfoTooltip>UNLOCKED и PAYABLE-потоки, которые ещё не имеют PAID.</InfoTooltip></div><div className="font-bold text-[#B8860B] mt-1">${metrics.periodPending.toLocaleString()}</div></div></div>
    ...
    <div className="bg-white min-w-0 p-4 md:p-6 rounded-xl border border-[#000052]/10">
      <div className="flex items-start gap-2 mb-5"><Wallet className="w-5 h-5 text-[#B8860B] shrink-0 mt-0.5" /><div className="min-w-0"><h2 className="text-lg font-bold text-[#000052] flex items-center break-words">{t('ui.agentExpectedIncome', 'Ожидаемый доход агентов')}<InfoTooltip>Полный payout по активному контракту агента.</InfoTooltip></h2><p className="text-sm text-[#000052]/60 break-words">Payout по активному контракту</p></div></div>
    ...
    <div className="bg-white min-w-0 p-4 md:p-6 rounded-xl border border-[#000052]/10">
      <div className="flex items-start gap-2 mb-5"><BarChart3 className="w-5 h-5 text-[#B8860B] shrink-0 mt-0.5" /><div className="min-w-0"><h2 className="text-lg font-bold text-[#000052] flex items-center break-words">{t('ui.agentPlanAchievement')}<InfoTooltip>Средний KPI агента по его контрактам.</InfoTooltip></h2><p className="text-sm text-[#000052]/60 mt-1 break-words">{t('ui.agentPlanDescription')}</p></div></div>
