...
const getPlanAchievement = (contract: any) => {
  const target = getTargetClients(contract);
  const actual = getActualClients(contract);
...
export function CEODashboard() {
  ...
  useEffect(() => {
    ...
    const loadMetrics = async () => {
      try {
        ...
        const efficiency: Record<string, { name: string; plan: number; targetClients: number; actualClients: number }> = {};
        ...
        activeContracts.forEach(contract => {
          ...
          if (!efficiency[key]) {
            efficiency[key] = {
              name: contract.agent_name || 'Агент',
              plan: 0,
              targetClients: 0,
              actualClients: 0,
            };
          ...
          efficiency[key].plan += revenue;
          efficiency[key].targetClients += getTargetClients(contract);
          efficiency[key].actualClients += getActualClients(contract);
        });

        realAgents.forEach(agent => {
          if (!efficiency[agent.id]) {
            efficiency[agent.id] = {
        ...
        setAgentEfficiency(
          Object.values(efficiency)
            .map(item => ({
              name: item.name,
              plan: item.plan,
              targetClients: item.targetClients,
              actualClients: item.actualClients,
              value: item.targetClients > 0 ? Math.round((item.actualClients / item.targetClients) * 100) : 0,
            }))
  ...
  const status = (value: number) => value < 80
    ? ['bg-red-500', 'Критично']
    : value > 100
      ? ['bg-emerald-500', 'Перевыполнение']
      : value === 100
        ? ['bg-emerald-500', 'План выполнен']
        : ['bg-[#B8860B]', 'Выполнение плана'];
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {kpis.map(([label, value, Icon, color, textColor, prefix, suffix], i) => (
          <div key={i} className={`${color} ${textColor} p-5 rounded-xl border border-[#000052]/10`}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-medium opacity-80">{label as string}</h3>
              <Icon className="w-5 h-5 opacity-80" />
            </div>
            <p className="text-2xl font-bold">{prefix as string}{Number(value).toLocaleString()}{suffix as string}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-[#000052]/10">
        <div className="flex items-start justify-between gap-4 mb-5">
          <div>
            ...
            <p className="text-sm text-[#000052]/60 mt-1">Эскроу, фактически выплаченные и ожидающие выплаты агентам</p>
          </div>
        ...
        <div className="h-5 bg-[#000052]/10 rounded-full overflow-hidden flex">
          ...
          <div className="h-full bg-[#B8860B] transition-all" style={{ width: `${100 - paymentProgress}%` }} />
        </div>

        <div className="grid grid-cols-3 gap-4 mt-4 text-sm">
          <div><span className="text-[#000052]/60">Эскроу</span><div className="font-bold text-[#000052] mt-1">${metrics.totalEscrow.toLocaleString()}</div></div>
          <div><span className="text-[#000052]/60">Выплачено в августе</span><div className="font-bold text-emerald-600 mt-1">${metrics.periodPaid.toLocaleString()}</div></div>
          <div><span className="text-[#000052]/60">Ожидает выплаты</span><div className="font-bold text-[#B8860B] mt-1">${metrics.periodPending.toLocaleString()}</div></div>
        </div>

        <div className="flex justify-between mt-3 text-xs text-[#000052]/60">
          <span>Выплачено: {paymentProgress}%</span>
          <span>Ожидает: {100 - paymentProgress}%</span>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-[#000052]/10">
        <div className="flex items-center gap-2 mb-5">
          ...
          <div>
            ...
            <p className="text-sm text-[#000052]/60">План продаж = 100% при создании контрактов. Выполнение считается по фактически привлечённым клиентам относительно целевого количества клиентов в этих контрактах.</p>
          </div>
        </div>

        <div className="flex flex-wrap gap-3 mb-6 text-xs text-[#000052]/70">
          <span>🔴 Ниже 80% — критично</span>
          <span>🟡 80–99% — выполнение плана</span>
          <span>🟢 100% — план выполнен</span>
          <span>🟢 Выше 100% — перевыполнение</span>
        </div>
        ...
        {agentEfficiency.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 items-end">
            {agentEfficiency.map((agent, index) => {
              const [bar, label] = status(agent.value);
              ...
              return (
                <div key={`${agent.name}-${index}`} className="flex flex-col items-center">
                  <div className="w-full h-[190px] flex items-end justify-center relative border-b border-[#000052]/10">
                    <div className={`w-12 sm:w-16 ${bar} rounded-t-lg relative`} style={{ height: `${height}px` }}>
                  ...
                  <div className="text-center mt-3 w-full">
                    ...
                    <div className="text-xs text-[#000052]/60 mt-1">{label}</div>
                    <div className="text-[11px] text-[#000052]/50 mt-1">
                      Клиенты: {agent.actualClients} / {agent.targetClients}
                    </div>
                    <div className="text-[11px] text-[#000052]/40 mt-0.5">
                      План договоров: ${agent.plan.toLocaleString()}
                    </div>
                  </div>
                </div>
              );
        ...
        {streamsByType.length === 0 ? (
          ...
          <div className="space-y-2">
            {streamsByType.map((stream, index) => {
              const percent = totalStreams ? stream.total / totalStreams * 100 : 0;
              return (
                <div key={stream.key} className="flex items-center gap-3">
                  <div className="w-3 h-3 rounded-full" style={{ backgroundColor: colors[index % colors.length] }} />
                  <div className="flex-1">
                    <div className="flex justify-between text-sm">
                      <span className="text-[#000052]">{stream.title}</span>
                      <b className="text-[#000052]">${stream.total.toLocaleString()}</b>
                    </div>
                    <div className="h-2 bg-[#000052]/10 rounded-full mt-1">
                      <div className="h-full rounded-full" style={{ width: `${percent}%`, backgroundColor: colors[index % colors.length] }} />
                    </div>
                  </div>
                  <span className="text-xs text-[#000052]/60 w-12 text-right">{percent.toFixed(1)}%</span>
                </div>
              );
            })}
          </div>
