...
export function CEODashboard() {
  ...
  return <div className="p-4 md:p-6 space-y-6">
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">{cards.map(([title, value, Icon, bg, fg, hint]: any, index: number) => <div key={index} className={`${bg} ${fg} p-5 rounded-xl border border-[#000052]/10`}><div className="flex items-center justify-between mb-3"><h3 className="text-sm font-medium opacity-80 flex items-center">{title}<InfoTooltip>{hint}</InfoTooltip></h3><Icon className="w-5 h-5 opacity-80" /></div><p className="text-2xl font-bold">{value}</p></div>)}</div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4"><div className="bg-white p-5 rounded-xl border border-[#000052]/10"><p className="text-sm text-[#000052]/60 flex items-center">{t('ui.locked', 'Заблокировано')}<InfoTooltip>Сумма потоков выплат со статусом LOCKED по активным контрактам. Эти средства ещё не доступны агенту.</InfoTooltip></p><p className="text-2xl font-bold text-[#B8860B] mt-1">${metrics.locked.toLocaleString()}</p><p className="text-xs text-[#000052]/50 mt-1">Средства заблокированы</p></div><div className="bg-white p-5 rounded-xl border border-[#000052]/10"><p className="text-sm text-[#000052]/60 flex items-center">{t('ui.released', 'Выплачено')}<InfoTooltip>Сумма потоков со статусом PAID. Это уже выплаченные агентам средства, а не просто сформированные обязательства.</InfoTooltip></p><p className="text-2xl font-bold text-emerald-600 mt-1">${metrics.released.toLocaleString()}</p><p className="text-xs text-[#000052]/50 mt-1">Средства выплачены</p></div><div className="bg-white p-5 rounded-xl border border-[#000052]/10"><p className="text-sm text-[#000052]/60 flex items-center">{t('ui.liability', 'Обязательства')}<InfoTooltip>Текущие обязательства по активным контрактам: LOCKED + UNLOCKED + PAYABLE. PAID сюда не входит.</InfoTooltip></p><p className="text-2xl font-bold text-[#000052] mt-1">${metrics.liability.toLocaleString()}</p><p className="text-xs text-[#000052]/50 mt-1">Текущие обязательства по выплатам</p></div></div>
    <div className="bg-white p-6 rounded-xl border border-[#000052]/10"><div className="flex items-start justify-between gap-4 mb-5"><div><div className="flex items-center gap-2"><Shield className="w-5 h-5 text-[#B8860B]" /><h2 className="text-lg font-bold text-[#000052]">{t('ui.cashMovementAugust')}</h2><InfoTooltip>Сравниваются выплаты текущего месяца и ожидаемые выплаты. Процент показывает долю уже выплаченных средств от суммы PAID + UNLOCKED/PAYABLE.</InfoTooltip></div><p className="text-sm text-[#000052]/60 mt-1">{t('ui.cashMovementDescription')}</p></div><div className="text-right"><div className="text-xs text-[#000052]/60 flex items-center justify-end">{t('ui.inEscrow')}<InfoTooltip>Общая сумма payout по активным контрактам, рассчитанная финансовым ядром.</InfoTooltip></div><div className="text-xl font-bold text-[#000052]">${metrics.totalEscrow.toLocaleString()}</div></div></div><div className="h-5 bg-[#000052]/10 rounded-full overflow-hidden flex"><div className="h-full bg-emerald-500" style={{ width: `${paymentProgress}%` }} /><div className="h-full bg-[#B8860B]" style={{ width: `${100 - paymentProgress}%` }} /></div><div className="grid grid-cols-3 gap-4 mt-4 text-sm"><div><div className="text-[#000052]/60 flex items-center">{t('ui.escrow')}<InfoTooltip>Сумма payout по активным контрактам. Годовой бонус в неё не включается.</InfoTooltip></div><div className="font-bold mt-1">${metrics.totalEscrow.toLocaleString()}</div></div><div><div className="text-[#000052]/60 flex items-center">{t('ui.paidInAugust')}<InfoTooltip>Сумма потоков со статусом PAID, дата которых относится к текущему месяцу.</InfoTooltip></div><div className="font-bold text-emerald-600 mt-1">${metrics.periodPaid.toLocaleString()}</div></div><div><div className="text-[#000052]/60 flex items-center">{t('ui.pendingPayout')}<InfoTooltip>Сумма потоков со статусом UNLOCKED или PAYABLE, которые ещё не имеют PAID.</InfoTooltip></div><div className="font-bold text-[#B8860B] mt-1">${metrics.periodPending.toLocaleString()}</div></div></div><div className="flex justify-between mt-3 text-xs text-[#000052]/60"><span>{t('ui.paid')}: {paymentProgress}%</span><span>{t('ui.pending')}: {100 - paymentProgress}%</span></div></div>
    <div className="bg-white p-6 rounded-xl border border-[#000052]/10"><div className="flex items-center gap-2 mb-5"><Wallet className="w-5 h-5 text-[#B8860B]" /><div><h2 className="text-lg font-bold text-[#000052] flex items-center">{t('ui.agentExpectedIncome', 'Ожидаемый доход агентов')}<InfoTooltip>Показывает месячный payout по активному контракту каждого агента. Это ожидаемая сумма, а не уже выплаченные деньги.</InfoTooltip></h2><p className="text-sm text-[#000052]/60">Месячный доход по активному контракту</p></div></div><div className="space-y-3">{expectedIncome.map((agent: any) => <div key={agent.name} className="flex items-center justify-between p-3 bg-[#000052]/5 rounded-lg"><span className="font-medium text-[#000052]">{agent.name}</span><span className="font-bold text-[#B8860B]">${agent.amount.toLocaleString()}</span></div>)}{expectedIncome.length === 0 && <p className="text-sm text-[#000052]/60">Нет контрактов</p>}</div></div>
    <div className="bg-white p-6 rounded-xl border border-[#000052]/10"><div className="flex items-center gap-2 mb-5"><Award className="w-5 h-5 text-[#B8860B]" /><div><h2 className="text-lg font-bold text-[#000052] flex items-center">{t('ui.annualBonusProgress', 'Годовой бонус')}<InfoTooltip>Накопление бонуса по выполнению годового плана продаж. Бонус отображается отдельно и не входит в escrow.</InfoTooltip></h2><p className="text-sm text-[#000052]/60">Накопление бонуса по выполнению годового плана продаж. Бонус не входит в эскроу.</p></div></div><div className="space-y-4">{annualBonuses.map((agent: any) => <div key={agent.id} className="p-4 bg-[#000052]/5 rounded-lg"><div className="flex items-center justify-between gap-4 mb-2"><div><div className="font-semibold text-[#000052]">{agent.name}</div><div className="text-xs text-[#000052]/60 flex items-center">{agent.planAchievementPercent}% {t('ui.planAchievement')}<InfoTooltip>Процент выполнения годового плана продаж по учтённым месяцам. Он используется для расчёта накопленного годового бонуса.</InfoTooltip></div></div><div className="text-right"><div className="font-bold text-[#B8860B]">${agent.accruedBonus.toLocaleString()} / ${agent.maxBonus.toLocaleString()}</div><div className="text-xs text-[#000052]/60 flex items-center justify-end">{agent.progressPercent}%<InfoTooltip>Доля максимально возможного годового бонуса, которая уже накоплена.</InfoTooltip></div></div></div><div className="h-3 bg-[#000052]/10 rounded-full overflow-hidden"><div className="h-full bg-[#B8860B] rounded-full" style={{ width: `${Math.min(agent.progressPercent, 100)}%` }} /></div></div>)}{annualBonuses.length === 0 && <p className="text-sm text-[#000052]/60">Нет агентов</p>}</div></div>
    <div className="bg-white p-6 rounded-xl border border-[#000052]/10"><div className="flex items-center gap-2 mb-5"><BarChart3 className="w-5 h-5 text-[#B8860B]" /><div><h2 className="text-lg font-bold text-[#000052] flex items-center">{t('ui.agentPlanAchievement')}<InfoTooltip>Средний KPI агента по его контрактам. Для реальных контрактов учитываются фактические показатели KPI, а при их отсутствии используется отношение фактической выручки к плановой.</InfoTooltip></h2><p className="text-sm text-[#000052]/60 mt-1">{t('ui.agentPlanDescription')}</p></div></div><div className="flex flex-wrap gap-4 mb-6 text-xs text-[#000052]/70"><span>{t('ui.below80')} — {t('ui.critical')}</span><span>100% — {t('ui.planAchievement')}</span><span>{t('ui.above100')} — {t('ui.overachieved')}</span></div><div className="overflow-x-auto"><div className="grid grid-cols-7 min-w-[1050px] border border-[#000052]/10 rounded-lg overflow-hidden">{efficiency.map((agent: any, index: number) => { const height = Math.max(8, Math.min(180, agent.value / 120 * 180)); return <div key={`${agent.name}-${index}`} className={`flex flex-col items-center min-w-0 px-3 py-4 ${index < efficiency.length - 1 ? 'border-r border-[#000052]/10' : ''}`}><div className="w-full h-[190px] flex items-end justify-center relative border-b border-[#000052]/10"><div className={`w-12 ${barClass(agent.value)} rounded-t-lg relative`} style={{ height: `${height}px` }}><span className="absolute -top-7 left-1/2 -translate-x-1/2 text-sm font-bold text-[#000052] whitespace-nowrap">{agent.value}%</span></div></div><div className="text-center mt-3 w-full"><div className="font-semibold text-sm text-[#000052] leading-tight" title={agent.name}>{agent.name}</div><div className="text-xs text-[#000052]/60 mt-1">{label(agent.value)}</div></div></div>; })}</div></div></div>
  </div>;
}
