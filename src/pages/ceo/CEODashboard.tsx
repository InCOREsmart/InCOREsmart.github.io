...
export function CEODashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // Демо-агенты и реальные агенты отображаются вместе.
  ...
  return <div className="p-4 md:p-6 space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div className="bg-white p-5 rounded-xl border border-[#000052]/10"><p className="text-sm text-[#000052]/60">{t('ui.locked', 'Р—Р°Р±Р»РѕРєРёСЂРѕРІР°РЅРѕ')}</p><p className="text-2xl font-bold text-[#B8860B] mt-1">${metrics.locked.toLocaleString()}</p><p className="text-xs text-[#000052]/50 mt-1">{t('ui.lockedDescription', 'РЎСЂРµРґСЃС‚РІР° Р·Р°Р±Р»РѕРєРёСЂРѕРІР°РЅС‹')}</p></div>
      <div className="bg-white p-5 rounded-xl border border-[#000052]/10"><p className="text-sm text-[#000052]/60">{t('ui.released', 'Р’С‹РїР»Р°С‡РµРЅРѕ')}</p><p className="text-2xl font-bold text-emerald-600 mt-1">${metrics.released.toLocaleString()}</p><p className="text-xs text-[#000052]/50 mt-1">{t('ui.releasedDescription', 'РЎСЂРµРґСЃС‚РІР° РІС‹РїР»Р°С‡РµРЅС‹')}</p></div>
      <div className="bg-white p-5 rounded-xl border border-[#000052]/10"><p className="text-sm text-[#000052]/60">{t('ui.liability', 'РћР±СЏР·Р°С‚РµР»СЊСЃС‚РІР°')}</p><p className="text-2xl font-bold text-[#000052] mt-1">${metrics.liability.toLocaleString()}</p><p className="text-xs text-[#000052]/50 mt-1">{t('ui.liabilityDescription', 'РўРµРєСѓС‰РёРµ РѕР±СЏР·Р°С‚РµР»СЊСЃС‚РІР° РїРѕ РІС‹РїР»Р°С‚Р°Рј')}</p></div>
    </div>
    ...
    <div className="bg-white p-6 rounded-xl border border-[#000052]/10"><div className="flex items-center gap-2 mb-5"><Wallet className="w-5 h-5 text-[#B8860B]" /><div><h2 className="text-lg font-bold text-[#000052]">{t('ui.agentExpectedIncome', 'РћР¶РёРґР°РµРјС‹Р№ РґРѕС…РѕРґ Р°РіРµРЅС‚РѕРІ')}</h2><p className="text-sm text-[#000052]/60">{t('ui.agentExpectedIncomeDescription', 'РќРµРІС‹РїР»Р°С‡РµРЅРЅС‹Рµ РїРѕС‚РѕРєРё РїРѕ Р°РєС‚РёРІРЅС‹Рј РѕР±СЏР·Р°С‚РµР»СЊСЃС‚РІР°Рј')}</p></div></div><div className="space-y-3">{expectedIncome.map((a: any) => <div key={a.name} className="flex items-center justify-between p-3 bg-[#000052]/5 rounded-lg"><span className="font-medium text-[#000052]">{a.name}</span><span className="font-bold text-[#B8860B]">${a.amount.toLocaleString()}</span></div>)}{expectedIncome.length === 0 && <p className="text-sm text-[#000052]/60">{t('ui.noExpectedIncome', 'РќРµС‚ РѕР¶РёРґР°РµРјС‹С… РІС‹РїР»Р°С‚')}</p>}</div></div>
    <div className="bg-white p-6 rounded-xl border border-[#000052]/10"><div className="flex items-center gap-2 mb-5"><Award className="w-5 h-5 text-[#B8860B]" /><div><h2 className="text-lg font-bold text-[#000052]">{t('ui.annualBonusProgress', 'Р“РѕРґРѕРІРѕР№ Р±РѕРЅСѓСЃ')}</h2><p className="text-sm text-[#000052]/60">{t('ui.annualBonusProgressDescription', 'РќР°РєРѕРїР»РµРЅРёРµ Р±РѕРЅСѓСЃР° РїРѕ РІС‹РїРѕР»РЅРµРЅРёСЋ РіРѕРґРѕРІРѕРіРѕ РїР»Р°РЅР° РїСЂРѕРґР°Р¶. Р‘РѕРЅСѓСЃ РЅРµ РІС…РѕРґРёС‚ РІ СЌСЃРєСЂРѕСѓ.')}</p></div></div><div className="space-y-4">{annualBonuses.map((a: any) => <div key={a.id} className="p-4 bg-[#000052]/5 rounded-lg"><div className="flex items-center justify-between gap-4 mb-2"><div><div className="font-semibold text-[#000052]">{a.name}</div><div className="text-xs text-[#000052]/60">{a.planAchievementPercent}% {t('ui.planAchievement', 'РїР»Р°РЅР°')}</div></div><div className="text-right"><div className="font-bold text-[#B8860B]">${a.accruedBonus.toLocaleString()} / ${a.maxBonus.toLocaleString()}</div><div className="text-xs text-[#000052]/60">{a.progressPercent}%</div></div></div><div className="h-3 bg-[#000052]/10 rounded-full overflow-hidden"><div className="h-full bg-[#B8860B] rounded-full" style={{ width: `${a.progressPercent}%` }} /></div></div>)}{annualBonuses.length === 0 && <p className="text-sm text-[#000052]/60">{t('ui.noAgents', 'РќРµС‚ Р°РіРµРЅС‚РѕРІ')}</p>}</div></div>
    <div className="bg-white p-6 rounded-xl border border-[#000052]/10"><div className="flex items-center gap-2 mb-5"><BarChart3 className="w-5 h-5 text-[#B8860B]" /><div><h2 className="text-lg font-bold text-[#000052]">{t('ui.agentPlanAchievement')}</h2><p className="text-sm text-[#000052]/60">{t('ui.agentPlanDescription')}</p></div></div><div className="flex flex-wrap gap-4 mb-6 text-xs text-[#000052]/70"><span>рџ”ґ {t('ui.below80')} вЂ” {t('ui.critical')}</span><span>рџџЎ 100% вЂ” {t('ui.planAchievement')}</span><span>рџџў {t('ui.above100')} вЂ” {t('ui.overachievement')}</span></div><div className="overflow-x-auto"><div className="grid grid-cols-7 min-w-[1050px] border border-[#000052]/10 rounded-lg overflow-hidden">{efficiency.map((a: any, i: number) => { const h = Math.max(8, Math.min(180, a.value / 120 * 180)); return <div key={`${a.name}-${i}`} className={`flex flex-col items-center min-w-0 px-3 py-4 ${i < efficiency.length - 1 ? 'border-r border-[#000052]/10' : ''}`}><div className="w-full h-[190px] flex items-end justify-center relative border-b border-[#000052]/10"><div className={`w-12 ${barClass(a.value)} rounded-t-lg relative`} style={{ height: `${h}px` }}><span className="absolute -top-7 left-1/2 -translate-x-1/2 text-sm font-bold text-[#000052] whitespace-nowrap">{a.value}%</span></div></div><div className="text-center mt-3 w-full"><div className="font-semibold text-sm text-[#000052] leading-tight" title={a.name}>{a.name}</div><div className="text-xs text-[#000052]/60 mt-1">{label(a.value)}</div></div></div>; })}</div></div></div>
  </div>;
}
