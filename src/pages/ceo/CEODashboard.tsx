...
import { getContractAccountingSnapshot, money } from '../../lib/contractFinance';
...
const realKPI = (contract: any) => {
  ...
  const planned = n(contract.revenue || contract.planned_revenue);
...
export function CEODashboard() {
    ...
    const load = async () => {
      try {
        ...
        const realContractsWithNames = realContracts.map(contract => ({
          ...
          is_demo: false,
          payout_streams: payoutStreams.filter(stream => stream.contract_id === contract.id),
        }));
        ...
        allContracts.forEach(contract => {
          const accounting = getContractAccountingSnapshot(contract);
          ...
          if (accounting.revenue > 0) {
            roiSum += accounting.revenue > 0 ? Math.round((accounting.companyProfit / accounting.revenue) * 100) : 0;
            roiCount++;
          }

          const streams: any[] = contract.payout_streams || [];
        ...
        const efficiencyRows = Object.values(efficiencyByAgent).map(agent => ({ name: agent.name, value: agent.count ? Math.round(agent.value / agent.count) : 0 })).sort((a, b) => b.value - a.value);
  ...
  const cards = [[t('ui.totalRevenue'), `$${metrics.totalRevenue.toLocaleString()}`, DollarSign, 'bg-[#000052]', 'text-white', 'GMV: сумма фактической выручки по активным контрактам.'], [t('ui.profit'), `$${metrics.netProfit.toLocaleString()}`, TrendingUp, 'bg-white', 'text-[#000052]', 'Прибыль компании: выручка минус полный payout агенту и комиссия InCORE.'], [t('ui.averageROI'), `${metrics.avgROI}%`, BarChart3, 'bg-[#1E3A5F]', 'text-white', 'Средний ROI: средняя доля прибыли компании в выручке активных контрактов.'], [t('ui.activeContracts'), String(metrics.activeContracts), CheckCircle, 'bg-white', 'text-[#000052]', 'Количество контрактов со статусом ACTIVE или IN_PROGRESS.']];
  ...
  return <div className="w-full min-w-0 overflow-x-hidden p-4 md:p-6 space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 min-w-0">
      <div className="bg-white min-w-0 p-5 rounded-xl border border-[#000052]/10"><p className="text-sm text-[#000052]/60 flex items-center break-words">{t('ui.locked', 'Заблокировано')}<InfoTooltip>Сумма потоков выплат со статусом LOCKED по активным контрактам.</InfoTooltip></p><p className="text-2xl font-bold text-[#B8860B] mt-1">${metrics.locked.toLocaleString()}</p><p className="text-xs text-[#000052]/50 mt-1">Средства заблокированы</p></div>
      <div className="bg-white min-w-0 p-5 rounded-xl border border-[#000052]/10"><p className="text-sm text-[#000052]/60 flex items-center break-words">{t('ui.released', 'Выплачено')}<InfoTooltip>Сумма потоков со статусом PAID.</InfoTooltip></p><p className="text-2xl font-bold text-emerald-600 mt-1">${metrics.released.toLocaleString()}</p><p className="text-xs text-[#000052]/50 mt-1">Средства выплачены</p></div>
      <div className="bg-white min-w-0 p-5 rounded-xl border border-[#000052]/10"><p className="text-sm text-[#000052]/60 flex items-center break-words">{t('ui.liability', 'Обязательства')}<InfoTooltip>Текущие обязательства по активным контрактам: LOCKED + UNLOCKED + PAYABLE.</InfoTooltip></p><p className="text-2xl font-bold text-[#000052] mt-1">${metrics.liability.toLocaleString()}</p><p className="text-xs text-[#000052]/50 mt-1">Текущие обязательства по выплатам</p></div>
    </div>
    <div className="bg-white min-w-0 p-4 md:p-6 rounded-xl border border-[#000052]/10">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-5"><div className="min-w-0"><div className="flex items-start gap-2"><Shield className="w-5 h-5 text-[#B8860B] shrink-0 mt-0.5" /><h2 className="text-lg font-bold text-[#000052] break-words">{t('ui.cashMovementAugust')}<InfoTooltip>Сравниваются выплаты текущего месяца и ожидаемые выплаты.</InfoTooltip></h2></div><p className="text-sm text-[#000052]/60 mt-1 break-words">{t('ui.cashMovementDescription')}</p></div><div className="text-left sm:text-right shrink-0"><div className="text-xs text-[#000052]/60 flex items-center sm:justify-end">{t('ui.inEscrow')}<InfoTooltip>Общая сумма payout по активным контрактам из финансового ядра.</InfoTooltip></div><div className="text-xl font-bold text-[#000052]">${metrics.totalEscrow.toLocaleString()}</div></div></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4 text-sm"><div><div className="text-[#000052]/60 flex items-center">{t('ui.escrow')}<InfoTooltip>Сумма payout по активным контрактам. Годовой бонус не включён.</InfoTooltip></div><div className="font-bold mt-1">${metrics.totalEscrow.toLocaleString()}</div></div><div><div className="text-[#000052]/60 flex items-center">{t('ui.paidInAugust')}<InfoTooltip>Сумма потоков PAID текущего месяца.</InfoTooltip></div><div className="font-bold text-emerald-600 mt-1">${metrics.periodPaid.toLocaleString()}</div></div><div><div className="text-[#000052]/60 flex items-center">{t('ui.pendingPayout')}<InfoTooltip>Сумма потоков UNLOCKED или PAYABLE.</InfoTooltip></div><div className="font-bold text-[#B8860B] mt-1">${metrics.periodPending.toLocaleString()}</div></div></div>
