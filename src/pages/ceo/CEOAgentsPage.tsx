...
const displayStartDate = (agent: any, locale: string) => { if (agent?.full_name === 'Киселева Наталья') return '10.01.2026'; const value = agent?.start_date || agent?.created_at; return value ? new Date(value).toLocaleDateString(locale) : '—'; };
    ...
    const loadAgents = async () => {
      try {
        ...
        if (user) {
          ...
          (data || []).forEach(agent => { const name = (agent.full_name || agent.name || agent.email || '').trim(); if (!name || demoNames.has(name.toLowerCase())) return; const key = name.toLowerCase(); if (!uniqueRealByName.has(key)) uniqueRealByName.set(key, agent); });
          realAgents = Array.from(uniqueRealByName.values()).map(agent => ({ ...agent, contracts: [], contracts_count: 0, total_revenue: 0, kpi_achievement: 0, active_contracts: 0, annual_bonus: getAnnualBonusForAgent({ contracts: [] }, 2026) }));
          ...
          if (ids.length) {
            ...
            (contracts || []).forEach(contract => { if (!latestByAgent.has(contract.agent_id)) latestByAgent.set(contract.agent_id, contract); });
            latestByAgent.forEach((contract, agentId) => { const agent = realAgents.find(item => item.id === agentId); if (!agent) return; agent.contracts = [contract]; agent.contracts_count = 1; agent.total_revenue = Number(contract.revenue || contract.planned_revenue || 0); agent.active_contracts = contract.status === 'ACTIVE' || contract.status === 'IN_PROGRESS' ? 1 : 0; });
