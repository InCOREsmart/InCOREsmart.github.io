...
const calculateRealAgentKPI = (contracts: any[]) => {
  ...
  const values = contracts.map(contract => {
    ...
    if (pairs.length > 0) {
      return Math.round(
        pairs.reduce(
          (sum, [actual, target]) => sum + (Number(actual || 0) / Number(target || 1)) * 100,
          0
        ) / pairs.length
      );
    }
    ...
    const actual = Number(contract.actual_revenue || contract.revenue || contract.sales_amount || 0);
        ...
        if (user) {
          ...
          if (ids.length > 0) {
            ...
            realAgents.forEach(agent => {
              agent.kpi_achievement = calculateRealAgentKPI(agent.contracts);
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div><h1 className="text-2xl md:text-3xl font-bold text-[#000052]">{t('nav.agents')}</h1><p className="text-sm text-[#000052]/70 mt-1">Управление командой страховых агентов</p></div>
        <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-[#B8860B] text-white rounded-lg hover:bg-[#9a7209] transition text-sm font-semibold"><Plus className="w-4 h-4" />{t('agent.addAgent')}</button>
      </div>
