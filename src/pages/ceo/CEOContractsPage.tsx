...
export function CEOContractsPage() {
  ...
  useEffect(() => {
    const fetchContracts = async () => {
      ...
      try {
        const demoContracts = [...DEMO_AGENTS.flatMap(agent => agent.contracts.map(contract => ({ ...contract, agent_name: agent.full_name, agent_id: agent.id, is_demo: true }))), ...COMPLETED_DEMO_CONTRACTS.map(contract => ({ ...contract, agent_name: DEMO_AGENTS.find(agent => agent.id === contract.id.split('-')[2])?.full_name || null, agent_id: contract.id.split('-')[2], is_demo: true }))];
