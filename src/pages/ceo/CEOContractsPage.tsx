...
export function CEOContractsPage() {
      ...
      try {
        ...
        if (companyData) {
          ...
          rawContracts.forEach(contract => {
            const agentName = agentsMap.get(contract.agent_id) || '';
            if (demoNames.has(agentName)) return;
          ...
          realContracts = realContracts.map(c => ({ ...c, agent_name: displayAgents.get(c.agent_id) || null, is_demo: false }));
        }
  ...
  const filteredContracts = contracts.filter(c => {
    ...
    return matchesQuery && (statusFilter === 'all' || c.status === statusFilter);
  });
