...
export function CEOContractsPage() {
  ...
  return <div className="p-4 md:p-8 space-y-6">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-5">{kpis.map((kpi, i) => { const Icon = kpi.icon; return <div key={i} className="bg-white p-5 rounded-2xl shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_12px_40px_rgba(0,0,82,0.12)]"><div className="flex items-start justify-between mb-4"><h3 className="text-sm font-medium text-gray-500 leading-tight">{kpi.label}</h3><div className={`w-10 h-10 rounded-[12px] flex items-center justify-center flex-shrink-0 ${kpi.box}`}><Icon className="w-5 h-5" /></div></div><Tooltip content={<div><div className="font-semibold mb-1">{kpi.label}</div><div>{kpi.sub}</div></div>}><p className={`text-[26px] font-bold tracking-tight cursor-help ${kpi.valueColor}`}>{kpi.value}</p></Tooltip><p className="text-xs text-gray-400 mt-1">{kpi.sub}</p></div>; })}</div>
