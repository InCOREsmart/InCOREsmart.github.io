...
export function CEODisputesPage() {
    ...
    const fetchDisputes = async () => {
      ...
      try {
        ...
        const detailed = await Promise.all(data.map(async d => {
          let agent_name = '—'; let contract_title = '—';
          if (d.agent_id) { const { data: agent } = await supabase.from('agents').select('full_name').eq('id', d.agent_id).maybeSingle(); agent_name = agent?.full_name || '—'; }
          if (d.contract_id) { const { data: contract } = await supabase.from('contracts').select('title').eq('id', d.contract_id).maybeSingle(); contract_title = contract?.title || '—'; }
          return { ...d, agent_name, contract_title };
        }));
        setDisputes(detailed);
      } catch (error) { console.error(error); setDisputes(DEMO_DISPUTES); } finally { setLoading(false); }
    };
    fetchDisputes();
  }, [user]);
  ...
  const handleResolve = async () => {
    ...
    try {
      if (selectedDispute.id.startsWith('demo-')) setDisputes(prev => prev.map(dispute => dispute.id === selectedDispute.id ? { ...dispute, status: 'RESOLVED', resolution: resolutionText } : dispute));
      else {
      ...
      setResolutionText(''); setSelectedDispute(null); alert(t('disputes.success'));
