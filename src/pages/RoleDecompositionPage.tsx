...
function normalizeRoleSkills(rows: SavedRoleSkill[]): Array<{ weight: number; is_required?: boolean; skills: SavedSkill | null }> {
  return rows.flatMap(row => {
    ...
    if (Array.isArray(skill)) {
      return skill.map(item => ({ weight: row.weight, is_required: row.is_required, skills: item }));
    }
    return [{ weight: row.weight, is_required: row.is_required, skills: skill }];
  });
}
  ...
  return (
    <div className="w-full min-w-0 p-4 md:p-8 space-y-6 max-w-7xl mx-auto overflow-x-hidden">
      <div className="flex items-start gap-3 min-w-0">
        ...
        <div className="min-w-0"><h1 className="text-[26px] md:text-3xl font-bold text-[#000052] break-words">Декомпозиция роли</h1><p className="text-sm text-gray-400 mt-1 break-words">AI конвертирует реальную работу роли в проверяемые навыки.</p></div>
      </div>
      ...
      {!result && (
        <div className="space-y-5 max-w-5xl min-w-0">
          <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-6 space-y-5 min-w-0">
            ...
            <div>
              <div className="hidden sm:grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_40px] gap-3 items-center mb-2"><div><label className="block text-sm font-semibold text-[#000052]">Что делает человек?</label><span className="text-xs text-gray-400">Конкретное действие</span></div><div><label className="block text-sm font-semibold text-[#000052]">Какой ожидаемый результат?</label><span className="text-xs text-gray-400">Что должно быть достигнуто</span></div><div /></div>
              <div className="space-y-3">{input.actions.map((item, index) => <div key={index} className="grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_40px] gap-2 sm:gap-3 items-start">
                <div className="min-w-0"><label className="sm:hidden block text-xs font-semibold text-[#000052] mb-1">Действие {index + 1}</label><input value={item} onChange={e => updateList('actions', index, e.target.value)} placeholder={`Действие ${index + 1}`} className="w-full min-w-0 px-4 py-2.5 border border-gray-200 rounded-xl text-[#000052]" /></div>
                <div className="relative min-w-0"><span className="hidden sm:block absolute -left-2 top-1/2 -translate-y-1/2 text-gray-300">→</span><label className="sm:hidden block text-xs font-semibold text-[#000052] mb-1">Ожидаемый результат</label><input value={input.expected_results[index] ?? ''} onChange={e => updateList('expected_results', index, e.target.value)} placeholder={`Результат ${index + 1}`} className="w-full min-w-0 px-4 py-2.5 border border-gray-200 rounded-xl text-[#000052] sm:pl-5" /></div>
                <button onClick={() => removeListItem(index)} disabled={input.actions.length === 1} className="justify-self-end sm:justify-self-auto p-2 text-gray-400 hover:text-red-600 disabled:opacity-30" aria-label={`Удалить действие ${index + 1}`}><Trash2 className="w-4 h-4" /></button>
              </div>)}</div>
      ...
      {result && validation && (
        <div className="space-y-5 min-w-0">
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden min-w-0"><div className="p-4 sm:p-5 border-b border-gray-100"><h3 className="font-bold text-[#000052]">Навыки роли</h3><p className="text-xs text-gray-400 mt-1 break-words">Изменяйте веса. Это значимость навыка внутри роли, а не распределение денег контракта.</p></div>
            <div className="hidden md:block overflow-x-auto"><table className="w-full min-w-[1050px]"><thead><tr className="bg-gray-50 text-xs text-gray-500 uppercase"><th className="text-left p-3">Навык</th><th className="text-left p-3 w-28">Вес</th><th className="text-left p-3">Верификация</th><th className="text-left p-3">Ожидаемый результат</th><th className="text-left p-3">Критерий проверки</th></tr></thead><tbody className="divide-y divide-gray-100">{result.skills.map((skill, index) => <tr key={skill.name} className="align-top"><td className="p-3"><div className="font-semibold text-[#000052] break-words">{skill.name}</div><div className="text-xs text-gray-500 mt-1 max-w-[280px] break-words">{skill.description}</div></td><td className="p-3"><div className="relative"><input type="number" min="0" max="100" step="1" value={(skill.weight * 100).toFixed(0)} onChange={e => updateWeight(index, e.target.value)} className="w-20 px-3 py-2 pr-7 border border-gray-200 rounded-lg font-bold text-[#B8860B]" /><span className="absolute right-3 top-2 text-[#B8860B]">%</span></div></td><td className="p-3 text-sm text-[#000052]">{VERIFICATION_LABELS[skill.verification_level]}</td><td className="p-3 text-sm text-gray-600">{skill.expected_outcomes.map(item => <div key={item}>• {item}</div>)}</td><td className="p-3 text-sm text-gray-600">{skill.verification_criteria.map(item => <div key={item}>• {item}</div>)}</td></tr>)}</tbody><tfoot><tr className="bg-gray-50 border-t-2 border-gray-200"><td className="p-3 font-bold text-[#000052]">Итого</td><td className={`p-3 font-bold ${Math.abs(result.skills.reduce((sum, skill) => sum + skill.weight, 0) - 1) < 0.001 ? 'text-emerald-600' : 'text-red-600'}`}>{(result.skills.reduce((sum, skill) => sum + skill.weight, 0) * 100).toFixed(0)}%</td><td colSpan={3}></td></tr></tfoot></table></div>
            <div className="md:hidden p-3 space-y-3">{result.skills.map((skill, index) => <article key={skill.name} className="rounded-xl border border-gray-100 bg-gray-50/60 p-4 min-w-0"><div className="flex items-start justify-between gap-3 min-w-0"><div className="min-w-0"><h4 className="font-bold text-[#000052] break-words">{skill.name}</h4><p className="text-xs text-gray-500 mt-1 break-words">{skill.description}</p></div><div className="relative shrink-0"><input type="number" min="0" max="100" step="1" value={(skill.weight * 100).toFixed(0)} onChange={e => updateWeight(index, e.target.value)} className="w-[74px] px-2 py-2 pr-6 border border-gray-200 rounded-lg font-bold text-[#B8860B] bg-white" aria-label={`Вес навыка ${skill.name} в процентах`} /><span className="absolute right-2 top-2 text-[#B8860B] pointer-events-none">%</span></div></div><div className="mt-3 space-y-3 text-sm"><div><div className="text-[11px] font-semibold uppercase tracking-wide text-gray-400">Верификация</div><div className="mt-1 font-medium text-[#000052] break-words">{VERIFICATION_LABELS[skill.verification_level]}</div></div><div><div className="text-[11px] font-semibold uppercase tracking-wide text-gray-400">Ожидаемый результат</div><div className="mt-1 text-gray-600 space-y-1 break-words">{skill.expected_outcomes.map(item => <div key={item}>• {item}</div>)}</div></div><div><div className="text-[11px] font-semibold uppercase tracking-wide text-gray-400">Критерий проверки</div><div className="mt-1 text-gray-600 space-y-1 break-words">{skill.verification_criteria.map(item => <div key={item}>• {item}</div>)}</div></div></div></article>)}<div className="rounded-xl bg-gray-50 border border-gray-100 p-4 flex items-center justify-between gap-3"><span className="font-bold text-[#000052]">Итого</span><span className={`font-bold ${Math.abs(result.skills.reduce((sum, skill) => sum + skill.weight, 0) - 1) < 0.001 ? 'text-emerald-600' : 'text-red-600'}`}>{(result.skills.reduce((sum, skill) => sum + skill.weight, 0) * 100).toFixed(0)}%</span></div></div>
          </div>

          <RoleMarketAnalysis skills={result.skills} />

          <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-5 min-w-0"><h3 className="font-bold text-[#000052] mb-3">Связи между навыками</h3>{result.skills_relations.length === 0 ? <p className="text-sm text-gray-400">Связей между навыками не обнаружено.</p> : <div className="space-y-2">{result.skills_relations.map((relation, index) => <div key={`${relation.skill_from}-${relation.skill_to}-${index}`} className="rounded-xl bg-gray-50 p-3 text-sm flex flex-wrap items-center gap-2 min-w-0"><span className="font-medium text-[#000052] break-words min-w-0">{relation.skill_from}</span><span className="shrink-0 px-2 py-1 bg-[#000052]/5 rounded-lg text-xs">{RELATION_LABELS[relation.relation_type]}</span><span className="font-medium text-[#000052] break-words min-w-0">{relation.skill_to}</span><span className="text-gray-400 shrink-0">{Math.round(relation.strength * 100)}%</span></div>)}</div>}</div>
      ...
      {!result && <section className="space-y-4 min-w-0"><div className="flex items-start sm:items-center justify-between gap-3"><div className="min-w-0"><h2 className="text-xl font-bold text-[#000052]">Сохранённые роли</h2><p className="text-sm text-gray-400 mt-1 break-words">Все сохранённые декомпозиции вашей компании.</p></div><button onClick={() => void loadSavedRoles()} className="shrink-0 p-2 rounded-xl hover:bg-[#000052]/5 text-[#000052]" title="Обновить" aria-label="Обновить"><RefreshCw className="w-4 h-4" /></button></div>{loadingRoles ? <div className="bg-white rounded-2xl p-6 text-sm text-gray-400">Загрузка ролей...</div> : savedRoles.length === 0 ? <div className="bg-white rounded-2xl p-8 text-center text-sm text-gray-400">Сохранённых ролей пока нет.</div> : <div className="grid gap-4 md:grid-cols-2">{savedRoles.map(role => <div key={role.id} className="bg-white rounded-2xl shadow-sm p-4 sm:p-5 border border-gray-100 min-w-0"><div className="flex items-start justify-between gap-3 min-w-0"><div className="min-w-0"><h3 className="font-bold text-[#000052] text-lg break-words">{role.name}</h3><p className="text-xs text-gray-400 mt-1 break-words">{role.industry}{role.category ? ` · ${role.category}` : ''}</p></div><span className="shrink-0 px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 text-xs font-semibold">Сохранена</span></div>{role.description && <p className="text-sm text-gray-500 mt-3 break-words">{role.description}</p>}<div className="mt-4 space-y-2">{normalizeRoleSkills(role.role_skills).map((item, index) => { const skillName = item.skills?.name; return skillName ? <div key={`${skillName}-${index}`} className="flex items-start justify-between gap-3 text-sm min-w-0"><span className="text-[#000052] break-words min-w-0">{skillName}</span><span className="font-bold text-[#B8860B] shrink-0">{Math.round(Number(item.weight) * 100)}%</span></div> : null; })}</div><div className="mt-5 pt-4 border-t border-gray-100 flex flex-col sm:flex-row gap-2"><button onClick={() => void openSavedRole(role)} disabled={openingRoleId === role.id} className="flex-1 px-3 py-2.5 rounded-xl border border-[#000052] text-[#000052] text-sm font-semibold disabled:opacity-50">{openingRoleId === role.id ? 'Открываем...' : 'Открыть роль'}</button><button onClick={() => navigate(`/ceo/contracts?roleId=${role.id}`)} className="flex-1 px-3 py-2.5 rounded-xl bg-[#000052] text-white text-sm font-semibold">Создать контракт</button></div></div>)}</div>}</section>}
    </div>
  );
}
