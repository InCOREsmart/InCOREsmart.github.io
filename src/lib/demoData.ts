...
function generateBitrixDeals(contract: Omit<DemoContract, 'bitrix_deals'>): BitrixDeal[] {
  ...
  for (let i = 0; i < Math.min(contract.actual_calls, 5); i++) {
    deals.push({
      id: `${contract.id}-call-${i + 1}`,
      title: `Звонок клиенту ${i + 1} (${month})`,
      stage: 'Новый',
      amount: 0,
      created_at: new Date(startDate.getTime() + i * 86400000).toISOString(),
    });
  }
  ...
  for (let i = 0; i < Math.min(contract.actual_proposals, 2); i++) {
    deals.push({
      id: `${contract.id}-proposal-${i + 1}`,
      title: `КП для клиента ${i + 1} (${month})`,
      stage: 'КП отправлено',
      amount: contract.revenue / contract.target_clients,
      created_at: new Date(startDate.getTime() + (i + 10) * 86400000).toISOString(),
    });
  }
...
function generateMonthlyContracts(
  ...
  while (currentMonth <= end) {
    ...
    const targetClients = Math.round(baseKpi.clients * variation);
    ...
    const status: 'ACTIVE' | 'IN_PROGRESS' | 'COMPLETED' = currentMonth < new Date() ? 'COMPLETED' : 'ACTIVE';
    ...
    const bitrixDeals = generateBitrixDeals(contractBase);
    
    contracts.push({
      ...contractBase,
      bitrix_deals: bitrixDeals,
    });
...
export const DEMO_AGENTS: DemoAgent[] = [
  {
    ...
    contracts: generateMonthlyContracts('demo-1', '2026-01-15', {
      calls: 120, meetings: 45, proposals: 30, clients: 10,
    }),
  },
  {
    id: 'demo-2',
    ...
    contracts: generateMonthlyContracts('demo-2', '2026-01-20', {
      calls: 150, meetings: 60, proposals: 40, clients: 12,
    }),
  },
  {
    id: 'demo-3',
    ...
    contracts: generateMonthlyContracts('demo-3', '2026-02-10', {
      calls: 90, meetings: 30, proposals: 20, clients: 8,
    }),
  },
  {
    id: 'demo-4',
    ...
    contracts: generateMonthlyContracts('demo-4', '2026-03-05', {
      calls: 130, meetings: 50, proposals: 35, clients: 9,
    }),
  },
  {
    id: 'demo-5',
    ...
    contracts: generateMonthlyContracts('demo-5', '2026-03-12', {
      calls: 80, meetings: 25, proposals: 18, clients: 5,
    }),
  },
  {
    id: 'demo-6',
    ...
    contracts: generateMonthlyContracts('demo-6', '2026-03-18', {
      calls: 160, meetings: 65, proposals: 45, clients: 14,
    }),
  },
  {
    id: 'demo-7',
    ...
    contracts: generateMonthlyContracts('demo-7', '2026-03-25', {
      calls: 110, meetings: 40, proposals: 28, clients: 7,
    }),
  },
  {
    id: 'demo-8',
    ...
    contracts: generateMonthlyContracts('demo-8', '2026-01-10', {
      calls: 140, meetings: 55, proposals: 38, clients: 11,
    }),
  },
];
...
export function calculateRevenueByMonth(): { month: string; value: number }[] {
  ...
  months.forEach((month) => {
    const activeContractsInMonth = DEMO_AGENTS.flatMap(agent => 
      agent.contracts.filter(contract => contract.month <= month.key)
    );
...
export function calculateTotalBitrixDeals(): number {
  return DEMO_AGENTS.reduce((sum, agent) => {
    return sum + agent.contracts.reduce((contractSum, contract) => {
      return contractSum + contract.bitrix_deals.length;
    }, 0);
  }, 0);
}