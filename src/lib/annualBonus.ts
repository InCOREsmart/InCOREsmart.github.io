...
import { getActualContractRevenue } from './contractFinance';
...
function getPlannedRevenue(contract: any): number { return Number(contract?.annual_sales_plan_monthly ?? contract?.target_revenue ?? contract?.kpi_revenue ?? contract?.planned_revenue ?? contract?.revenue ?? 0); }
...
export function calculateAnnualBonusProgress(contracts: Array<DemoContract | any>, year = new Date().getFullYear()): AnnualBonusProgress {
  ...
  const explicitAnnualTarget = yearContracts.reduce((sum, contract) => { const annualTarget = Number(contract.annual_sales_plan || contract.year_sales_plan || 0); return sum + (annualTarget > 0 ? annualTarget : 0); }, 0);
  ...
  const actualAnnualSales = yearContracts.reduce((sum, contract) => sum + getActualContractRevenue(contract), 0);
  const planAchievement = annualTarget > 0 ? clamp(actualAnnualSales / annualTarget) : 0;
  const months = new Set(yearContracts.map(contract => { const dateValue = contract.start_date || contract.start_at || contract.created_at; const date = new Date(dateValue); return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`; }));
