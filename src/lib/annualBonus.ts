...
function getPositiveNumber(...values: unknown[]): number {
...
export function calculateAnnualBonusProgress(
  ...
  const explicitAnnualTarget = yearContracts.reduce((sum, contract) => {
    const annualTarget = getPositiveNumber(contract.annual_sales_plan, contract.year_sales_plan);
    return sum + annualTarget;
  }, 0);
  ...
  // Only confirmed/realized sales count toward the annual bonus.
