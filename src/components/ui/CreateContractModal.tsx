  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: contract, error: contractError } = await supabase.from('contracts').insert({
        ...
        actual_property_revenue: calculations.propertyRevenue,
        actual_casco_revenue: calculations.cascoRevenue,
        actual_dms_revenue: calculations.dmsRevenue,
        actual_renewal_revenue: calculations.renewalRevenue,
        actual_cross_sell_revenue: calculations.crossSellRevenue,
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl border border-[#000052]/10 my-8">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div><label className="block text-sm font-semibold text-[#000052] mb-1.5">Агент *</label><select value={agentId} onChange={e => setAgentId(e.target.value)} className="w-full px-4 py-2.5 bg-white border border-[#000052]/20 rounded-lg text-[#000052] focus:outline-none focus:ring-2 focus:ring-[#B8860B]/30" required><option value="">Выберите агента</option>{agents.map(agent => <option key={agent.id} value={agent.id}>{agent.full_name} ({agent.specialization || 'без специализации'})</option>)}</select></div>
          ...
          <div className="bg-gradient-to-br from-[#000052] to-[#000052]/90 text-white p-6 rounded-xl"><h3 className="text-lg font-bold mb-4 flex items-center gap-2"><Calculator className="w-5 h-5 text-[#B8860B]" />Итоговый расчёт</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="space-y-2"><div className="text-sm font-semibold text-[#B8860B] mb-2">Общая сумма договоров:</div><div className="text-xl font-bold mb-4">${calculations.totalContractRevenue.toLocaleString()}</div><div className="text-sm font-semibold text-[#B8860B] mb-2">Эскроу (замороженные средства):</div>{[['Новые продажи', calculations.bonusNewSales], ['Продление', calculations.bonusRenewal], ['Кросс-продажи', calculations.bonusCrossSell], ['Бонус за план', calculations.bonusPlan], ['Удержание 90 дней', calculations.bonusRetention]].map(([label, amount]) => <div key={String(label)} className="flex justify-between text-sm"><span className="opacity-80">{label}</span><span className="font-bold">${(amount as number).toLocaleString()}</span></div>)}</div><div className="space-y-3 border-l border-white/20 pl-4"><div className="flex justify-between"><span className="opacity-80">Итого эскроу</span><span className="text-2xl font-bold text-[#B8860B]">${calculations.totalEscrow.toLocaleString()}</span></div><div className="flex justify-between text-sm"><span className="opacity-80">Комиссия InCORE ({PLATFORM_FEE_PERCENT}%)</span><span className="font-bold">${calculations.platformFee.toLocaleString()}</span></div><div className="flex justify-between text-sm"><span className="opacity-80">Выплата агенту</span><span className="font-bold">${calculations.agentPayout.toLocaleString()}</span></div><div className="flex justify-between text-sm"><span className="opacity-80">Результат компании</span><span className="font-bold text-[#B8860B]">${calculations.companyProfit.toLocaleString()}</span></div><div className="flex justify-between text-sm"><span className="opacity-80">ROI</span><span className="font-bold text-[#B8860B]">{calculations.roi}%</span></div></div></div></div>
