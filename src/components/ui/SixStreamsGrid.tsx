...
export function SixStreamsGrid({
  streams,
  amounts = {},
}: SixStreamsGridProps) {
  ...
  const completionPercent =
    totalStreams > 0
      ? Math.round((paidStreams / totalStreams) * 100)
      : 0;
  ...
  const formatAmount = (stream: PaymentStream) => {
    ...
    if (stream.fixedAmount) {
      return `$${amount.toLocaleString()}`;
    }

    if (stream.type === 'annual') {
      return '1/12';
    }

    if (stream.type === 'clawback') {
      return `$${amount.toLocaleString()}`;
    }
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {streams.map((stream) => {
          ...
          const Icon =
            streamIcons[stream.id] || TrendingUp;

          const hasClawback =
            stream.type === 'clawback';

          const isCompleted =
            (amounts[stream.id] || 0) > 0;
