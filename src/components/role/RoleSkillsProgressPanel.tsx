...
interface ContractProgress { status: string; actualClients: number; targetClients: number; actualCalls: number; targetCalls: number; actualMeetings: number; targetMeetings: number; actualProposals: number; targetProposals: number; actualRevenue: number; plannedRevenue: number; isDemo: boolean; }
...
function numberFromPayload(payload: unknown, keys: string[]) {
  ...
  for (const key of keys) { const value = Number(source[key]); if (Number.isFinite(value) && value > 0) return value; }
  return 0;
}
...
export function RoleSkillsProgressPanel({ roleId, contractId }: RoleSkillsProgressPanelProps) {
    ...
    const load = async () => {
      ...
      try {
        ...
        const { data: contractData, error: contractError } = await supabase.from('contracts')
          .select('role_id, status, target_clients_new, revenue, planned_revenue')
        ...
        const [{ data: roleData, error: roleError }, { data: oracleData, error: oracleError }] = await Promise.all([
          effectiveRoleId ? supabase.from('roles').select('name, role_skills(weight, skills(name, description, verification_level, expected_outcomes, verification_criteria))').eq('id', effectiveRoleId).maybeSingle() : Promise.resolve({ data: null, error: null }),
          supabase.from('oracle_events').select('event_type, payload').eq('contract_id', effectiveContractId),
        ]);

        if (roleError || !roleData) { setRoleName('Страховой агент'); setSkills(DEMO_SKILLS); }
        else {
        ...
        const actualClients = oracleEvents.reduce((max, event: any) => Math.max(max, numberFromPayload(event.payload, ['actual_clients', 'clients', 'actualClients'])), 0);
        const actualRevenue = oracleEvents.reduce((max, event: any) => Math.max(max, numberFromPayload(event.payload, ['actual_contract_revenue', 'actual_revenue', 'contract_revenue', 'revenue', 'amount'])), 0);
        setEvents(new Set(oracleEvents.map((event: any) => event.event_type)));
        setContractProgress({
          status: contractData.status || '', actualClients, targetClients: Number(contractData.target_clients_new || 0),
          actualCalls: 0, targetCalls: 0, actualMeetings: 0, targetMeetings: 0, actualProposals: 0, targetProposals: 0,
          actualRevenue, plannedRevenue: Number(contractData.planned_revenue || contractData.revenue || 0), isDemo: false,
        });
  ...
  const operationalProgress = useMemo(() => {
    ...
    const values = [ratio(contractProgress.actualCalls, contractProgress.targetCalls), ratio(contractProgress.actualMeetings, contractProgress.targetMeetings), ratio(contractProgress.actualProposals, contractProgress.targetProposals), salesProgress].filter((value) => value > 0);
  ...
  const progressForSkill = (skillName: string) => {
    ...
    if (name.includes('план') || name.includes('plan')) return events.has('PLAN_ACHIEVED') ? 100 : planProgress;
  ...
  const stageProgress = completed ? 100 : weightedProgress;
  ...
  return (
    <section data-role-progress-panel className="w-full max-w-4xl mx-auto bg-white border border-gray-100 rounded-2xl shadow-sm px-4 py-4 md:px-5 md:py-5">
      <div className="flex items-center justify-between gap-4 mb-3">
        ...
        <div className="shrink-0 text-right"><div className="text-xl font-bold text-[#000052] leading-none">{weightedProgress}%</div><div className="text-[10px] text-gray-400 mt-1">фактический прогресс</div></div>
      </div>
      <div className="mb-4"><div className="flex items-center justify-between text-[11px] mb-1.5"><span className="font-semibold text-[#000052]">Этап контракта: {stageLabel}</span><span className="text-gray-400">{stageProgress}%</span></div><div className="h-2 bg-gray-100 rounded-full overflow-hidden"><div className="h-full bg-[#B8860B] rounded-full transition-all" style={{ width: `${stageProgress}%` }} /></div></div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
        <div className="rounded-lg bg-[#000052]/5 px-3 py-2"><div className="text-[10px] text-gray-400">Договоры</div><div className="text-sm font-bold text-[#000052]">${Number(contractProgress?.actualRevenue || 0).toLocaleString()} / ${Number(contractProgress?.plannedRevenue || 0).toLocaleString()}</div></div>
        ...
        <div className="rounded-lg bg-[#000052]/5 px-3 py-2"><div className="text-[10px] text-gray-400">План продаж</div><div className="text-sm font-bold text-[#000052]">{planProgress}%</div></div>
        <div className="rounded-lg bg-[#000052]/5 px-3 py-2"><div className="text-[10px] text-gray-400">Статус</div><div className="text-sm font-bold text-[#000052]">{accepted ? 'В работе' : completed ? 'Выполнен' : 'Подготовка'}</div></div>
      </div>
      <div className="divide-y divide-gray-100">
        {skills.map((item, index) => {
          const skill = item.skills; if (!skill) return null;
          const progress = progressForSkill(skill.name); const weight = Number(item.weight || 0); const displayWeight = weight <= 1 ? weight * 100 : weight;
          return <div key={`${skill.name}-${index}`} className="py-2.5 first:pt-0 last:pb-0"><div className="flex items-center justify-between gap-3 mb-1"><span className="text-sm font-semibold text-[#000052] truncate">{skill.name}</span><span className="shrink-0 text-[11px] text-gray-400">Вес {displayWeight}% · {progress}%</span></div><div className="h-1.5 bg-gray-100 rounded-full overflow-hidden"><div className="h-full bg-[#B8860B] rounded-full transition-all" style={{ width: `${progress}%` }} /></div>{skill.description && <p className="text-[11px] text-gray-500 mt-1">{skill.description}</p>}{skill.verification_criteria?.length ? <p className="text-[10px] text-gray-400 mt-0.5"><span className="font-semibold">Критерии:</span> {skill.verification_criteria.join(' · ')}</p> : null}</div>;
        })}
      </div>
      <div className="mt-4 pt-3 border-t border-gray-100"><div className="flex items-center gap-2 text-xs font-semibold text-[#000052] mb-2"><Activity className="w-3.5 h-3.5 text-[#B8860B]" />KPI контракта</div><div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        <div className="rounded-lg bg-gray-50 px-3 py-2"><div className="text-[10px] text-gray-400">Звонки</div><div className="text-sm font-bold text-[#000052]">{contractProgress?.actualCalls || 0}/{contractProgress?.targetCalls || 0}</div></div>
        <div className="rounded-lg bg-gray-50 px-3 py-2"><div className="text-[10px] text-gray-400">Встречи</div><div className="text-sm font-bold text-[#000052]">{contractProgress?.actualMeetings || 0}/{contractProgress?.targetMeetings || 0}</div></div>
        <div className="rounded-lg bg-gray-50 px-3 py-2"><div className="text-[10px] text-gray-400">КП</div><div className="text-sm font-bold text-[#000052]">{contractProgress?.actualProposals || 0}/{contractProgress?.targetProposals || 0}</div></div>
        <div className="rounded-lg bg-gray-50 px-3 py-2"><div className="text-[10px] text-gray-400">Клиенты</div><div className="text-sm font-bold text-[#000052]">{contractProgress?.actualClients || 0}/{contractProgress?.targetClients || 0}</div></div>
      </div></div>
    </section>
  );
}
