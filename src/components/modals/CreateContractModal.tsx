...
import { X, Calculator, Users, ChevronDown, Target, Phone, Handshake, FileText, TrendingUp, ShieldCheck } from 'lucide-react';
...
export function CreateContractModal({ isOpen, onClose, onCreated }: CreateContractModalProps) {
  ...
  useEffect(() => {
    ...
    const newSalesBonus = rev * 0.50; // 50% от GMV
    const renewalBonus = (typeof renewalAmount === 'number' ? renewalAmount : 0) * 0.15; // 15% от продления
    const crossSellBonus = (typeof crossSellAmount === 'number' ? crossSellAmount : 0) * 0.10; // 10% от кросс-продаж
    const planBonus = rev * 0.10; // 10% от GMV за выполнение плана
    const retentionBonus = rev * 0.10; // 10% от GMV за удержание
    const annualBonus = rev * 0.05; // 5% от GMV годовой бонус
    
    const totalAgentBonuses = newSalesBonus + renewalBonus + crossSellBonus + planBonus + retentionBonus + annualBonus;
    const escrow = totalAgentBonuses; // Все бонусы в эскроу
          ...
          <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
            ...
            {rev > 0 && (
              <div className="mb-6 p-4 bg-white rounded-lg border border-gray-200">
                <p className="text-xs font-bold text-[#000052] uppercase mb-3">6 потоков выплат агенту (эскроу)</p>
                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-xs">
                    <div className="w-2 h-2 rounded-full bg-[#000052]"></div>
                    <span className="flex-1 text-gray-700">Новые продажи (50%)</span>
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      <span className="flex-1 text-gray-700">Продление (15% от суммы)</span>
                    </div>
                    <input
                      type="number"
                      value={renewalAmount}
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      <span className="flex-1 text-gray-700">Кросс-продажи (10% от суммы)</span>
                    </div>
                    <input
                      type="number"
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="flex-1 text-gray-700">Бонус за выполнение плана (10%)</span>
                    <span className="font-semibold text-[#000052]">${economics.planBonus.toLocaleString()}</span>
                  </div>

                  <div className="flex items-center gap-2 text-xs">
                    <div className="w-2 h-2 rounded-full bg-yellow-500"></div>
                    <span className="flex-1 text-gray-700">Удержание 90 дней (10%)</span>
                    <span className="font-semibold text-[#000052]">${economics.retentionBonus.toLocaleString()}</span>
                  </div>

                  <div className="flex items-center gap-2 text-xs">
                    <div className="w-2 h-2 rounded-full bg-purple-500"></div>
                    <span className="flex-1 text-gray-700">Годовой бонус (5%)</span>
                    <span className="font-semibold text-[#000052]">${economics.annualBonus.toLocaleString()}</span>
                  </div>
                </div>
