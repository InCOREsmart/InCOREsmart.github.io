...
const detectContactType = (value: string): 'email' | 'telegram' | 'other' => { if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) return 'email'; if (/^(https?:\/\/)?(t\.me\/|telegram\.me\/|@)/i.test(value)) return 'telegram'; return 'other'; };
...
export function HrReportLeadCapture({ t, input, result, scenarioReduction = 0, scenarioRemainingLoss, totalLoss, potentialEffect, benchmarkRatio = 0, benchmarkStatus = 'unknown', onSubmit }: Props) {
  ...
  const [contact, setContact] = useState(''); const [submitted, setSubmitted] = useState(false); const [saving, setSaving] = useState(false); const [error, setError] = useState(false);
  const submit = async (event: React.FormEvent) => { event.preventDefault(); const value = contact.trim(); if (!value || saving) return; setSaving(true); setError(false); const locale = (i18n.resolvedLanguage || i18n.language || 'ru').split('-')[0]; const diagnosis = result ? diagnoseHrLoss(result) : null; const remainingLoss = scenarioRemainingLoss ?? result?.totalLoss ?? 0; const effect = result ? Math.max(0, result.totalLoss - remainingLoss) : 0; const { error: insertError } = await supabase.from('hr_calculator_leads').insert({ contact: value, contact_type: detectContactType(value), locale, total_loss: result?.totalLoss ?? 0, potential_effect: result ? effect : 0, source: 'hr-calculator', primary_driver: diagnosis?.primaryDriver ?? null, primary_driver_share: diagnosis?.primaryShare ?? 0, benchmark_ratio: benchmarkRatio, benchmark_status: benchmarkStatus, scenario_reduction_percent: scenarioReduction, scenario_remaining_loss: remainingLoss, inputs: input ?? {}, }); if (insertError) { setSaving(false); setError(true); return; } onSubmit?.(value);
    try {
