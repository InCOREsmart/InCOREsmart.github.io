...
export const AuthProvider=({children}:{children:ReactNode})=>{
 ...
 const determineRole=async(u:User):Promise<UserRole>=>{const meta=(u.user_metadata?.role as string|undefined)?.toLowerCase();if(meta==='guest')return'guest';if(meta==='agent'||meta==='ceo')return meta;const {data:c}=await supabase.from('companies').select('id').eq('user_id',u.id).maybeSingle();if(c)return'ceo';const {data:a}=await supabase.from('agents').select('id').eq('user_id',u.id).maybeSingle();if(a)return'agent';return'ceo';};
